...
import { PlusCircle, Trash2, User, LogOut, Loader, Layout } from 'lucide-react';
...
export default function MyCanvasesPage() {
  ...
  const fetchCanvases = useCallback(async () => {
    ...
    try {
      ...
      const userCanvases = data.map((canvas: any) => {
    ...
    <div className="min-h-screen w-full bg-background text-foreground p-4 md:p-8">
      <header className="flex justify-between items-center mb-12">
        ...
        <div className="flex items-center gap-4">
          <Link href="/dashboard">
            <Button variant="secondary"><Layout className="mr-2" />Dashboard</Button>
          </Link>
          <Link href="/profile">
